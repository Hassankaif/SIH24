    ...
    while read -r l_out; do
        if [ -n "$l_out" ]; then
            if [[ $l_out =~ ^\s*# ]]; then
                ...
                if grep -Piq -- "^\h*$l_systemd_parameter_name\b" <<< "$l_systemd_parameter"; then
                    A_out["$l_systemd_parameter"]="$l_file"
                fi
            fi
        fi
    done < <(
        /usr/bin/systemd-analyze cat-config "$l_systemd_config_file" |
        grep -Pio '^\h*([^#\n\r]+|#\h*\/[^#\n\r\h]+\.conf\b)'
    )
    ...
    if (( ${#A_out[@]} > 0 )); then
        ...
        while IFS="=" read -r l_systemd_file_parameter_name l_systemd_file_parameter_value; do
            ...
            if grep -Piq "^\h*$l_systemd_file_parameter_value\b" <<< "$l_systemd_file_parameter_value"; then
                ...
                l_output2="$l_output2\n - \"$l_systemd_file_parameter_name\" is incorrectly set to \"$l_systemd_file_parameter_value\" in \"$(printf '%s' "${A_out[@]}")\" and should have a value matching: \"$l_systemd_file_parameter_value\"\n"
            fi
